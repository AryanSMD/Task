...
const total = computed(() => Math.floor(store.getTotalFroms / 9) + 1);
...
function next() {
    !isLastPage.value && currentPage.value++;
}

function previous() {
    !isFirstPage.value && currentPage.value--;
}
</script>
        ...
        <div class="w-1/3 h-full flex items-center justify-between">
            ...
            <div class="w-1/3 h-full flex justify-center items-center gap-3">
                ...
                <input 
                    ...
                    v-model="currentPage"
                    @change="store.getAllForms(currentPage, 9)"
                >
            </div>
